...
3. The agent scans your repo for stack fingerprints, infers values, and **proposes one unified diff**.
4. You review and approve (`yes` / `edit` / `abort`). The agent writes only after approval and flips status to `initialized`.
...
node scripts/init-backbone.mjs . --write --yes  # write after you reviewed it
```
...
Useful flags: `--force` (overwrite existing files), `--dry-run` (preview without writing).
...
│   └── settings.json
│
├── .cursor/                      ← Cursor surface (rules/, commands/)
...
│
├── scripts/                      ← mvck CLI, init-backbone, daily-enhance, validate-kit
...
### Skills (`skills/` is canonical; `.claude/skills` and `.agents/skills` mirror it)
...
Expected result: validation passes, AgentShield probe reports no critical structural issues, and `npm pack --dry-run` shows the intended package files.
