...
- CLI surfaces (Claude Code, Codex CLI, Kimi CLI terminals): emit the ASCII wave view.
- Emit both when the surface is unknown or mixed.
- The renderer validates the ledger (unknown nodes, duplicates, cycles) before rendering, and its output is deterministic: after any graph change, regenerate instead of editing the output.
...
- the rendered graph view (Mermaid, ASCII, or both) generated by `scripts/render-graph.mjs`.
