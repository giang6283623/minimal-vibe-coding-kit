...
`latest-brief.json` uses `tutien-response-brief-v1`. It carries policy state, safe project identity/stack/validation commands, aggregate evidence, classification/progression slugs, bounded findings without event IDs, optional villain identity and voice metadata, the effective fictional-humiliation profile, an optional story pointer, and composition constraints. The composition metadata declares the persistent active-mode response scope, exact preservation of literal technical content, immersive xianxia voice, cutting evidence-bound villain sarcasm, the hard boundary between a consented fictional avatar and any real person, and a role-address firewall that keeps internal target labels out of final prose. Its object shape is not a response layout.
