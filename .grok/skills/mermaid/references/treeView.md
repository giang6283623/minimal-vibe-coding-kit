> **Upstream reference**
>
> Adapted from [Mermaid 11.16.0 source](https://github.com/mermaid-js/mermaid/blob/mermaid%4011.16.0/packages/mermaid/src/docs/syntax/treeView.md). This kit copy is
...
Set a node's icon explicitly with `icon(name)`, where `name` is any icon from a registered [icon pack](https://github.com/mermaid-js/mermaid/blob/mermaid%4011.16.0/packages/mermaid/src/docs/config/icons.md), referenced as `pack:name`. Explicit icons always render, even when `showIcons` is off:
