...
function isKitManagedPath(file) {
  const managedRoots = new Set(['.claude', '.cursor', '.agents', '.codex', '.codex-plugin', 'skills', 'commands']);
  const managedFiles = new Set([
    ...
    'backbone.yml',
    'scripts/mvck.mjs',
    'scripts/init-backbone.mjs',
    'scripts/daily-enhance.mjs',
    'scripts/validate-kit.mjs',
    'scripts/doctor.mjs',
    'scripts/test-install.mjs',
    'scripts/agentshield-probe.mjs',
...
if (shouldWrite) {
  ...
  console.log('To write it after approval, run: node scripts/init-backbone.mjs . --write --yes');
}
