...
- Keep root instructions concise and move long procedures into `skills/` or `docs/`.
- Do not add package lifecycle scripts, hooks, MCP servers, migrations, or deploy steps that run during install.
- Update `docs/backbone.schema.json` and `scripts/validate-kit.mjs` together when `backbone.yml` changes.
- Add an install/idempotency test when changing `scripts/mvck.mjs`.
