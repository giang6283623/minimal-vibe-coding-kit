...
Select controller settings from live evidence while keeping provider choice,
model choice, transport, and controller ownership separate.
...
runtime's currently exposed structured question mechanism. Common tool labels
are examples, not assumptions. If no structured tool is available, ask one
concise plain-text question at a time. Child agents return `needs_user_input`
and never question the user directly.
...
2. **Transport.** If the provider has more than one ready route, distinguish
   application, CLI, SDK, API, MCP, and manual routes. State billing and data
   boundaries that differ.
3. **Model.** Fetch a fresh authenticated catalog for the selected route. Offer
   `provider-default` plus two verified exact model IDs when useful. Revalidate
   free-form IDs against the same catalog.
4. **Reasoning effort.** Ask only when the selected route attests supported
   effort values for that model. Omit the question when the provider controls
   effort automatically or cannot attest it.
5. **Consent.** Confirm any change in provider billing, data exposure,
   permission scope, or mutation authority before the first controller request.
...
3. the host dispatches native workers or executes sequentially under host
   permissions;
